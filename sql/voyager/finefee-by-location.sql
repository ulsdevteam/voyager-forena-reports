...
select l.location_code "Circ_Loc",                  
	          SUM(CASE f.trans_type WHEN 9 THEN f.trans_amount / 100 else 0 END) "Accured_Demerit",
		  SUM(CASE f.trans_type WHEN 8 THEN f.trans_amount / 100  ELSE 0 END) "Accured_Fine",
...
where (l.library_id = 2 or l.library_id = 1 or l.library_id = 3 or l.library_id = 5 or l.library_id = 6 or l.library_id = 7 or l.library_id = 9 or l.library_id = 10) and
	...
	trans_date BETWEEN TRUNC(TRUNC(SYSDATE, 'MONTH') -1, 'MONTH') AND TRUNC(SYSDATE, 'MONTH') - .00001
GROUP BY
	l.location_code 
	ORDER BY
	l.location_code
